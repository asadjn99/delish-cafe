...
import { User, Mail, Phone, Printer, ChevronLeft, ChevronRight } from "lucide-react";
...
const AboutHero: React.FC = () => {
  const images = ["/images/insta1.jpeg", "/images/insta2.jpeg", "/images/insta3.jpeg"];
    ...
    <section className="relative w-full h-[400px] md:h-[500px] overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-20">
        ...
        <p className="text-[10px] md:text-xs font-bold text-gray-300 uppercase tracking-widest">
          <Link href="/" className="hover:text-[#cf432d] transition-colors">
            Home
          </Link>
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 p-2 rounded-full backdrop-blur-md text-white z-30"
      >
        <ChevronLeft size={22} />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 p-2 rounded-full backdrop-blur-md text-white z-30"
      >
...
// ==========================
const MemberDetails: React.FC<{ id: string }> = ({ id }) => {
  const member: TeamMember | undefined = teamMembers.find((m) => m.id === id);
        ...
        <div className="bg-white shadow-sm border border-gray-100 p-6 md:p-12 flex flex-col-reverse lg:flex-row gap-10 items-start">
          ...
          <div className="w-full lg:w-7/12 flex flex-col gap-6">
            <div className="space-y-2 text-center lg:text-left">
              <h1 className="text-3xl md:text-5xl font-black text-gray-900">{member.name}</h1>
              <p className="text-sm md:text-base text-[#cf432d] font-medium tracking-wide uppercase">{member.role}</p>
            </div>
            <p className="text-gray-500 leading-relaxed text-sm md:text-base text-center lg:text-left">{member.bio}</p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-4 py-8 my-2">
              <DetailItem icon={User} label="Experience" value={member.experience} />
              <DetailItem icon={Mail} label="Email" value={member.email} />
              <DetailItem icon={Phone} label="Phone" value={member.phone} />
              <DetailItem icon={Printer} label="Fax" value={member.fax} />
            </div>

            <div className="flex justify-center lg:justify-start">
              <button className="bg-[#cf432d] hover:bg-[#cd6150] text-white font-bold py-3 px-8 text-sm uppercase tracking-wider">
                Contact Me
              </button>
            </div>
          </div>
